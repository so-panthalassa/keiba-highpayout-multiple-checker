...
def fetch_jra_shutuba(
    ...
    直近/適性/コンビ特徴を付与して返す。horse_id / race_id は不使用。
    """
    # HTML
...
# ===== あなたの add_market_features をそのまま利用 =====
...
# ============= 特徴量作成を強化 =============
# 学習時カラム（あなたの提示リスト）を fallback としておく
BASE_FEATURES = [
    '枠番','斤量','騎手','馬体重','調教師','course_len','weather','race_type',
    'ground_state','競馬場','所属','月','馬体重_増減','性','年齢','出走頭数','相対枠位置',
    '大外枠','最内枠','馬_直近3走_複勝率','馬_直近5走_複勝率','馬_直近10走_複勝率','馬_直近5走_平均着順',
...
def align_dtypes_for_lgbm(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in obj_cols:
        if c in df.columns:
            df[c] = df[c].astype(str)  # object 扱いに揃える
...
def predict_from_url(
    ...
    """
    # 1) 出馬表取得（history_df を渡すと馬名マッチの履歴特徴が付与される
    ...
    # 2) 特徴リスト＆カテゴリ列をロード（JSONにあればそれを最優先）
    ...
    # 予測（validate_features=FalseのままでOK）
    try:
        p_lgbm = np.asarray(lgbm.predict_proba(X_lgbm_native, validate_features=False))[:, 1]
    except Exception as e:
    ...
    cats_eff = list(cats_cat)  # or list(cats_cat_eff)
...
# ★ 予測関数が同ファイル内に定義済み or import 済みであることが前提
# from your_module import predict_from_url
...
st.set_page_config(page_title="馬券妙味チェッカー", layout="wide")
st.title("馬券の妙味チェッカー（試作）")
...
if go and url.strip():
    ...
    # 3) 見送り率が無ければフォールバックで再計算 → 注目度 = 1 - 見送り率
